"""Session helpers — extracted from the now-deleted session/loop.py.

Provides the shared constants and utility functions that were previously
defined in loop.py but are still needed by interactive.py and commands.py.

Functions related to the InteractiveSession itself live in interactive.py.
"""
...
def build_get_well_phase_list() -> list[dict]:
    ...
    triage = {
        ...
        "prompt": (
            ...
            "OUTPUT FORMAT:\n"
            "Write a structured triage document using ## File: headings so your "
            "output is captured as a real file. Format like:\n"
            "    ## File: triage.md\n"
            "    ## Finding Summary\n"
            "    ...\n\n"
            "Use sections:\n"
    ...
    requirements = {
        ...
        "prompt": (
            ...
            "OUTPUT FORMAT:\n"
            "Write a structured requirements document using ## File: headings so your "
            "output is captured as a real file. Format like:\n"
    ...
    design = {
        ...
        "prompt": (
            ...
            "OUTPUT FORMAT:\n"
            "Write a structured design document using ## File: headings so your "
            "output is captured as real files. The main doc goes in one file, "
            "and each ADR gets its own file. Format like:\n"
            "    ## File: docs/arch/remediation-design.md\n"
            "    ## Design Overview & Strategy\n"
            "    ...\n"
            "    ## File: docs/arch/adr-001-extract-cycle-runner.md\n"
            "    ## ADR-001: Extract Cycle Runner\n"
            "    ...\n\n"
            "Use sections:\n"
            "    ## Design Overview & Strategy\n"
...
def _apply_file_blocks(root: Path, text: str) -> list[tuple[str, str]]:
    ...
    for filepath, content in files.items():
        ...
        if not str(full_path).startswith(str(root_resolved)):
            results.append((filepath, "error: path escapes project root"))
            continue
...
def _report_apply_results(results: list[tuple[str, str]], root: Path) -> None:
...
def _do_consult(
    ...
    from harness.agents.consultation import ConsultationCapability
...
# ── Cycle result display helpers ────────────────────────────────────────────


def _process_cycle_result_for_display(cycle_result) -> list[str]:
    ...
    if hasattr(cycle_result, 'error') and cycle_result.error:
        lines.append(f"  Error: {cycle_result.error}")
    if hasattr(cycle_result, 'is_phase_jump') and cycle_result.is_phase_jump:
        target = cycle_result.jump_target or "unknown"
        lines.append(f"  \U0001f500 Phase jump requested \u2192 {target}")
        if hasattr(cycle_result, 'summary') and cycle_result.summary:
            lines.append(f"    Reason: {cycle_result.summary}")
    elif hasattr(cycle_result, 'summary') and cycle_result.summary:
...
def _check_phase_jump_limit(
    ...
    Returns True if the jump is allowed, False if the limit is exceeded.
    """
...
def _format_jump_marker(cycle_result) -> str:
...
def _check_for_phase_jump_from_content(content: str) -> str | None:
...
def read_session_type(root: Path, slug: str) -> str | None:
    """Read session type from the engagement's ``engagement.yaml``.

    Validates against known session types. Returns ``None`` if not yet
    set or if the stored value is invalid (defaults to GREENFIELD downstream).
    """
    import yaml
    from harness.paths import get_engagement_yaml
...
def confirm_session_type(suggested: str) -> str | None:
    ...
    """
    labels = {
        "refactoring": "refactoring (restructure code toward ideal architecture)",
        "brownfield": "brownfield (work within existing code, document compromises)",
    }
...
def _prompt_alternative(rejected: str) -> str | None:
