...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1 | Move arg parsers from repl.py to _registration.py | ✅ Complete | ~16 functions |
| 2 | Add `@register(click_only=True)` to all Click-only commands | ✅ Complete | ~11 commands |
| 3 | Replace `COMMAND_TYPES` with `build_repl_command_map()` | ✅ Complete | ~200 lines deleted |
| 4 | Replace `/help` with REGISTRY-based generation (grouped) | ✅ Complete | Crichton notes #2, #3 |
| 5 | Add "CLI only" fallback to REPL dispatch | ✅ Complete | Crichton note #1 |
| 6 | Remove dead arg parsers + unused imports from repl.py | ✅ Complete | Cleanup |
