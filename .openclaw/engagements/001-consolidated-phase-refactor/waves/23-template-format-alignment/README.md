...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1 | FMT-1: Add `input:` to all 9 simple templates | 📋 Pending | Templates can't participate in data-flow without it |
| 2 | FMT-5: Add `loop:` example to phases.yaml | 📋 Pending | Exercise the critic loop code path that's currently untested |
| 3 | FMT-2: Update Step.role docstring | 📋 Pending | Document loop-only semantics |
| 4 | FMT-4: Add sub_steps to LoopConfig | 📋 Pending | Remove context-injection hack from template_registry and step_executor |
